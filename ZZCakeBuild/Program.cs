...
public sealed class ExportTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        ...
        var modsDirectory = Path.Combine(appDataPath, "VintagestoryData", "Mods");
        
        if (!Directory.Exists(modsDirectory))
        {
            Console.WriteLine($"Vintage Story mods directory not found at: {modsDirectory}. Skipping export.");
            return;
        }

        var zipFile = $"../Releases/{context.Name}_{context.Version}.zip";
        context.CopyFile(zipFile, Path.Combine(modsDirectory, $"{context.Name}_{context.Version}.zip"));
        Console.WriteLine($"Exported mod to: {modsDirectory}");
    }
}

[TaskName("Default")]
[IsDependentOn(typeof(ExportTask))]
